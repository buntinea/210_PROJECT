...
def messageFunction (client, userdata, message):
    ...
    if topic == "MoistureReading":
        ...
        if moisture >= moistureThreshold:
            ...
            weather = "wet"
            #print('the window is locked closed because of the rain')
        else:
            tooWet = False
            weather = "dry"
            #print(moisture)
        ...
        elif outsideTemp > insideTemp: #if outside is warmer than inside...
...
def getGui():
    ...
    def updateMoisture():
        ...
        #yourTempValue['text'] = idealTemp
        #yourTempValue.config(text=idealTemp)
    ...
    tempOutValue.grid(row=2,column=1)
    tempInValue = tkinter.ttk.Label(window, text = insideTemp)
    tempInValue.grid(row=2,column=2)
